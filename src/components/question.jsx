...
function App() {
  ...
  const structuredQuestion = {
    question:
      "Explain how React Native differs from React for web development?",
  ...
  const handleSubmit = (e) => {
    ...
    if (questionType === "multiple") {
      setScore(
        parseInt(currentAnswer) === multipleChoiceQuestion.correctAnswer
          ? 100
          : 0
      );
    } else {
      ...
      setScore(
        Math.round(
          (matches.length / structuredQuestion.correctKeywords.length) * 100
        )
      );
    }
  };
          ...
          {score !== null && (
            <div className="result-container">
              ...
              {questionType === "structured" && (
                <p className="hint">
                  Hint: Include keywords like{" "}
                  {structuredQuestion.correctKeywords.join(", ")}
                </p>
              )}
            </div>
          )}
